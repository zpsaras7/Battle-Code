...
>>>>>>> 03c18dbb70b6bc9b9b5c844743b7ce24be4f2574
			...
			try {
				...
				if (robots.length > 0) {
					...
					if (rc.canFireSingleShot()) {
						...
						//try to move in previous direction with 10 degree possible spread on either side
						tryMove(getRandomDirTargeted((short) lastMovedDirection.getAngleDegrees(), (short) 10) );
					}
				}
				else {
	 ...
	 */
	public static short tryToWater() throws GameActionException{
		//TreeInfo[] waterableNearbyTrees = new TreeInfo[0];
		short ans = 0;
		if(rc.canWater()) { //5
			TreeInfo[]nearbyTrees = rc.senseNearbyTrees(); //100
			int weakestTreeID = 0;
